...
.tg .tg-7t53{background-color:#ffffff;border-color:#ffffff;font-family:"Times New Roman", Times, serif !important;font-size:12px;
...
.tg .tg-o20t{border-color:#ffffff;font-family:serif !important;font-size:20px;text-align:center;vertical-align:top;font-weight:bold;}
...
<tbody>
  <tr>
    <td class="tg-o20t" colspan="10">INFORME DE VENTAS TOTALES</td>
  </tr>
    <tr>
  <td class="tg-o20t1" colspan="10">de  {{$fecha_inicio}} al {{$fecha_fin}}<br><b>Tipo Movimiento </b>{{$tipo_movimiento}}</td>
    </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-6ccv">Usuario</td>
    <td class="tg-6ccv">Nro. Venta</td>
    <td class="tg-6ccv">Movimiento</td>
    <td class="tg-6ccv">Cliente</td>   
    <td class="tg-6ccv">Observacion</td>
    <td class="tg-6ccv">Efectivo</td>
    <td class="tg-6ccv">QR</td>
    <td class="tg-6ccv">Total</td>
  </tr>
  @foreach ($ventas as $venta)
    <tr>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $loop->iteration }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->venta_fecha }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->nombre }}</td>        
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->numero_venta }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->tipo }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->cliente }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->observacion }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->efectivo }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->qr }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $venta->total }}</td>

...
<table class="tg" width=100%><thead>
  <tr>
    <th class="tg-a9mw" colspan="8"></th>
  </tr></thead>
<tbody>
  <tr>
    <td class="tg-o20t" colspan="8">PRODUCTOS MAS VENDIDOS</td>
  </tr>
  <tr>
    <td class="tg-o20t1" colspan="8">de  {{$fecha_inicio}} al {{$fecha_fin}}<br><b>Tipo Movimiento</b> {{$tipo_movimiento}}</td>
  </tr>
  <tr>
  ...
  @foreach ($ventas_productos as $ventas_producto)
    ...
    <tr>
        ...
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->codigo }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->producto }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->concentracion }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->marca }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->presentacion }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->tipo_movimiento }}</td>
        <td class="{{ $loop->odd ? 'tg-6c9e' : 'tg-rcbp' }}">{{ $ventas_producto->total_vendido }}</td>
